...
export default function Home() {
	...
	const [chunks, setChunks] = useState<PGResource[]>([]);
	...
	const handleSearch = async () => {
		...
		const results: PGResource[] = await searchResponse.json();
    ...
    `;
		...
		if (!answerResponse.ok) {
			setLoading(false);
			console.log(answerResponse.statusText);
		}
				...
				<div className="flex-1 overflow-auto">
					<div className="mx-auto flex h-full w-full max-w-[750px] flex-col items-center px-3 pt-4 sm:pt-8">
						...
						{loading ? (
							...
							<div className="mt-6 text-center text-lg">{`AI-powered search & chat for India's laws.`}</div>
						)}
					</div>
				</div>
				<Footer />
